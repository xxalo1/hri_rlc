...
def array_equal(a: ArrayT, b: ArrayT, rtol: float = 1e-05, atol: float = 1e-08) -> bool:
    """Check if two arrays/tensors are element-wise equal within a tolerance."""
    if isinstance(a, np.ndarray) and isinstance(b, np.ndarray):
        return np.array_equal(a, b)
    elif isinstance(a, torch.Tensor) and isinstance(b, torch.Tensor):
        return torch.equal(a, b)
    raise NotImplementedError(f'allclose not implemented for type {type(a)}')