...
std::string Self::getDescription() const { return kPlannerDescription; }
...
bool Self::canServiceRequest(
  ...
  const bool planner_id_ok =
      req.planner_id.empty() || (req.planner_id == kPlannerId);
  if (!planner_id_ok) {
    return false;
  }
...
planning_interface::PlanningContextPtr Self::getPlanningContext(
  ...
  auto context = std::make_shared<TrajOptPlanningContext>(
      context_name, req.group_name, node_, parameter_namespace_);
