...
std::string Robot::strip_prefix_if_present(const std::string& name, const std::string& prefix)
{
...
void Robot::configure_io(const std::vector<std::string>& src_names, const std::string& prefix)
{
  if (!prefix.empty())
    joint_prefix_ = prefix;
  else 
  ...
  if (static_cast<int>(src_names.size()) != n)
  {
    throw std::runtime_error(
      "configure_io: src_names.size() != dyn.n(): got " +
      std::to_string(src_names.size()) + " expected " + std::to_string(n));
  }
  ...
  {
    const std::string canon = strip_prefix_if_present(src_names[i], joint_prefix_);
    if (src_index.find(canon) != src_index.end())
    {
      throw std::runtime_error("configure_io: duplicate joint name after normalization: " + canon);
    }
...
void Robot::to_model_inplace(const Eigen::Ref<const Vec>& src, Eigen::Ref<Vec> out_model) const
{
...
void Robot::to_src_inplace(const Eigen::Ref<const Vec>& model, Eigen::Ref<Vec> out_src) const
{
...
void Robot::set_joint_des(
  const Vec* q_des_src,
  const Vec* qd_des_src,
  const Vec* qdd_des_src
)
{
  if (q_des_src)
    to_model_inplace(*q_des_src, q_des_model_);
  if (qd_des_src)
    to_model_inplace(*qd_des_src, qd_des_model_);
  if (qdd_des_src)
    to_model_inplace(*qdd_des_src, qdd_des_model_);
}

void Robot::set_base_pose_wb(const Eigen::Ref<const Vec>& pose_wb)
{
  if (pose_wb.size() != 7)
    throw std::runtime_error("set_base_pose_wb: pose must be size 7 [x y z qw qx qy qz]");

  pose_wb_ = pose_wb;

  const Eigen::Quaterniond q_wb(
    pose_wb_[3], pose_wb_[4], pose_wb_[5], pose_wb_[6]); // (w,x,y,z)
...
)
{
  ...
  if (N <= 0)
    throw std::runtime_error("set_joint_traj: empty trajectory");

  if (traj_src.q.cols() != n || traj_src.qd.cols() != n || traj_src.qdd.cols() != n)
  {
    throw std::runtime_error("set_joint_traj: q/qd/qdd must be (N, n_src) with n_src == dyn.n()");
  }
  if (traj_src.q.rows() != N || traj_src.qd.rows() != N || traj_src.qdd.rows() != N)
  {
...
void Robot::sample_joint_traj_into_desired_()
{
  ...
  while (traj_i1_ < N && t_arr[traj_i1_] <= t_)
    ++traj_i1_;
  ...
  for (int i = 0; i < n; ++i)
  {
    ...
    qdd_des_model_[i] = a0 * traj_model_.qdd(i0, i) + a1 * traj_model_.qdd(i1, i);
  }
}

void Robot::update_joint_des_from_traj()
{
  if (trace_mode_ == TraceMode::HOLD)
    return;
...
const Robot::Vec& Robot::compute_ctrl_effort()
{
  ...
  if (trace_mode_ == TraceMode::TRAJ)
    update_joint_des_from_traj();

  switch (ctrl_mode_)
  {
    case CtrlMode::CT:
      ctrl_.computed_torque(
        q_model_, qd_model_,
        q_des_model_, qd_des_model_, qdd_des_model_,
        tau_model_);
      break;
    ...
    case CtrlMode::PID:
    {
      ...
      ctrl_.pid(q_model_, qd_model_, q_des_model_, qd_des_model_, dt, tau_model_);
      break;
    }
