...
Dynamics Dynamics::FromUrdf(const std::string &urdf, UrdfSource source,
                            const std::string &tcp_frame) {
  if (source == UrdfSource::Path) return FromUrdfPath(urdf, tcp_frame);
  if (source == UrdfSource::Xml) return FromUrdfXml(urdf, tcp_frame);
  throw std::runtime_error("Unsupported UrdfSource in Dynamics::FromUrdf");
}

Dynamics Dynamics::FromUrdfPath(const std::string &urdf_path,
                            const std::string &tcp_frame) {
...
Dynamics Dynamics::FromUrdfXml(const std::string &urdf_xml,
                               const std::string &tcp_frame) {
  pinocchio::Model model;
...
void Dynamics::set_qd(const Eigen::Ref<const Vec> &qd) { qd_ = qd; }

void Dynamics::set_qdd(const Eigen::Ref<const Vec> &qdd) { qdd_ = qdd; }

void Dynamics::step(const Vec *q_dof, const Vec *qd, const Vec *qdd) {
  if (q_dof) set_q(*q_dof);
  if (qd) set_qd(*qd);
  if (qdd) set_qdd(*qdd);
}
