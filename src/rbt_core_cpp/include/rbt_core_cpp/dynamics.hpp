...
/// @brief Identifies how a URDF is provided to `Dynamics::FromUrdf`.
enum class UrdfSource {
  /// @brief Interpret the URDF argument as a filesystem path.
  Path,
  /// @brief Interpret the URDF argument as an in-memory URDF XML string.
  Xml,
};

/**
 * @brief Rigid-body dynamics wrapper around Pinocchio model/data with cached
 * computations.
 *
 * @details
 * Size conventions:
 * - `nq()`: Pinocchio configuration size (`model.nq`) used by inverse dynamics
 * (`rnea()`).
 * - `n() == nv()`: number of velocity DOFs (`model.nv`) used for per-joint
 * vectors and matrices.
 *
 * Joint vectors passed to `set_q()`, `set_qd()`, and `set_qdd()` are in *model
 * canonical order* with `size = n()`. Internally, positions are expanded into
 * the Pinocchio configuration vector `q()`
 * (`size = nq()`), including handling of continuous joints (`nq() != n()`).
 *
 * Cached computations are invalidated by `set_q()` and recomputed lazily on
 * demand.
 *
 * @par Thread safety
 * Not thread-safe.
...
class Dynamics final {
 public:
   ...
   * @param[in] model Pinocchio model to take ownership of (moved into this
   * instance).
   * @param[in] tcp_frame Optional TCP frame name; if empty, uses the last frame
   * in the model.
   * @warning If `tcp_frame` is non-empty and not present in the model,
   * `tcp_frame_id()` will be invalid and calls using it (e.g., `frame_T()`) may
   * throw.
   */
  explicit Dynamics(pinocchio::Model model, std::string tcp_frame = "");

  /**
   * @brief Builds a Pinocchio model from a URDF path or XML string and
   * constructs `Dynamics`.
   * @param[in] urdf URDF input, interpreted according to `source`.
   * @param[in] source URDF source selector.
   * @param[in] tcp_frame Optional TCP frame name; if empty, uses the last frame
   * in the model.
   * @return A `Dynamics` instance owning the parsed model.
   * @throws std::exception If URDF parsing fails (thrown by Pinocchio).
   */
  static Dynamics FromUrdf(const std::string& urdf, UrdfSource source,
   ...
   * @throws std::exception If URDF parsing fails (thrown by Pinocchio).
   */
  static Dynamics FromUrdfPath(const std::string& urdf_path,
                               const std::string& tcp_frame = "");

  /**
   * @brief Builds a Pinocchio model from an in-memory URDF XML string and
   * constructs `Dynamics`.
   * @param[in] urdf_xml URDF XML content.
   * @param[in] tcp_frame Optional TCP frame name; if empty, uses the last frame
   * in the model.
   * @return A `Dynamics` instance owning the parsed model.
   * @throws std::exception If URDF parsing fails (thrown by Pinocchio).
   */
  static Dynamics FromUrdfXml(const std::string& urdf_xml,
                              const std::string& tcp_frame = "");
   ...
   * @pre `q.size() == n()`.
   * @details Positions are expanded into the Pinocchio configuration vector
   * `q()` (`size = nq()`), including `cos/sin` handling for continuous joints.
   */
  void set_q(const Eigen::Ref<const Vec>& q);

  /**
   * @brief Sets joint velocities.
   * @param[in] qd Joint velocities [rad/s], size = `n()` (model canonical
   * order).
   * @pre `qd.size() == n()`.
   */
   ...
   * @param[in] qdd Joint accelerations [rad/s^2], size = `n()` (model canonical
   * order).
   * @pre `qdd.size() == n()`.
   */
   ...
   * @param[in] q_dof Optional joint positions [rad], size = `n()`; may be
   * nullptr (no update).
   * @param[in] qd Optional joint velocities [rad/s], size = `n()`; may be
   * nullptr (no update).
   * @param[in] qdd Optional joint accelerations [rad/s^2], size = `n()`; may be
   * nullptr (no update).
   */
  ...
  /// @brief Computes the joint-space mass matrix for the stored configuration
  /// `q()`.
  void compute_crba();

  /// @brief Computes generalized gravity torques for the stored configuration
  /// `q()`.
   ...
   * @note The returned reference points to an internal cache that is recomputed
   * on demand after `set_q()`.
   */
  const Mat& M();
   ...
   * @note The returned reference points into Pinocchio `Data` and is recomputed
   * on demand after `set_q()`.
   */
  const Vec& g();
   ...
   * @return Frame index. If the name is unknown, Pinocchio returns an invalid
   ...
   * @note The returned reference is invalidated by subsequent kinematics
   * updates.
   */
   ...
   * @return Frame Jacobian, shape = (6, `n()`), mapping joint velocities to
   * spatial velocity.
   * @note The returned reference points to an internal cache and is overwritten
   * on the next call.
   */
  const Mat6& frame_jacobian(
