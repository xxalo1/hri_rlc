 ...
 *   then converted once into *model canonical order* for fast sampling at 1
 * kHz.
 ...
 * - Designed for 1 kHz control loop (no heap allocations in
 * compute_ctrl_effort()).
 *
 * Threading:
 * - Not thread-safe by design (you said you will keep Robot mutated by one
 * thread).
 */
class Robot final {
 public:
  ...
  // ------------------------
  /**
   * @brief Compute control effort in *source joint order* (matches actuator
   * ordering).
   *
   * This function:
